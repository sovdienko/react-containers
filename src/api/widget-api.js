...
export function getWidgets() {
  return axios.get('http://localhost:3004/widgets')
    .then(response => {
...
export function searchWidgets(query = '') {
  return axios.get('http://localhost:3004/widgets?q='+query)
    .then(response => {
        store.dispatch(getWidgetsSuccess(response.data));
        return response;
    });
...
export function deleteWidget(widgetId) {
  return axios.delete('http://localhost:3004/widgets/' + widgetId)
    .then(response => {
